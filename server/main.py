...
def get_all_sensors_info():
    db = SessionLocal()
    try:
        # Subquery to get latest time per sensor
        subquery = (
            db.query(
                NumericalValue.sensor_name,
                func.max(NumericalValue.time).label("latest_time")
            )
            .group_by(NumericalValue.sensor_name)
            .subquery()
        )

        # Join with main table to get latest value per sensor
        SensorAlias = aliased(NumericalValue)
        latest_values = (
            db.query(SensorAlias)
            .join(
                subquery,
                (SensorAlias.sensor_name == subquery.c.sensor_name) &
                (SensorAlias.time == subquery.c.latest_time)
            )
            .all()
        )
